...
public class PostReadController {
    ...
    @Operation(
            summary = "게시글 제목 검색",
            description = """
                    제목 키워드를 기반으로 게시글을 검색합니다.

                    [요청 경로]
                    - GET /api/v1/post-read/search
                    ...
                    [응답 필드]
                    - content: 게시글 요약 정보 리스트 (PostSummaryResDto[])
                    - totalPages, totalElements 등 페이징 정보 포함

                    [처리 로직]
                    - deletedStatus가 false인 게시글 중 제목에 keyword가 포함된 게시글을 검색합니다.
                    - 대소문자를 구분하지 않고 검색합니다.
    ...
    @GetMapping("/search")
    public BaseResponseEntity<PostListPageResponseDto> searchPostsByTitle(
            @RequestParam String keyword,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "8") int size
    ) {
        return new BaseResponseEntity<>(postReadService.searchPostsByTitle(keyword, page, size));
    }
}
