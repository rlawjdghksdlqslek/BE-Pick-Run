...
public class CommentLikeController {
    ...
    @Operation(
            summary = "회원 댓글 좋아요 여부 확인",
            description = """
            특정 댓글에 대해 회원이 좋아요를 눌렀는지 여부를 확인합니다.

            [요청 경로]
            - GET /api/v1/comment-like/{commentUuid}/likes/me

            [요청 헤더]
            - X-Member-UUID: (String, optional) 회원 UUID. 비회원일 경우 null 처리

            [응답 필드]
            - liked: (boolean) true: 좋아요한 경우 / false: 좋아요하지 않은 경우
        """
    )
    @GetMapping("{commentUuid}/likes/me")
    public BaseResponseEntity<CommentLikeCheckResDto> hasLikedComment(
            @PathVariable String commentUuid,
            @RequestHeader(value = "X-Member-UUID", required = false) String memberUuid
    ) {
        return new BaseResponseEntity<>(commentLikeService.hasLikedComment(commentUuid, memberUuid));
    }
}
