...
public class KafkaConsumerConfig {
    ...
    @Bean
    public Map<String, Object> postUpdateConsumerConfigs() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServer);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "post-updated-read-group");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, PostUpdatedEvent.class);

        return props;
    }

    @Bean
    public ConsumerFactory<String, PostUpdatedEvent> postUpdatedEventConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(
                postUpdateConsumerConfigs(),
                new StringDeserializer(),
                new ErrorHandlingDeserializer<>(new JsonDeserializer<>(PostUpdatedEvent.class, false))
        );
    }
